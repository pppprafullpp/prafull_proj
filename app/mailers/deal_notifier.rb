      ...
      if sp.service_category_id == 1
        ...
        @telephone_t_deal = TrendingDeal.where("category_id = ?", sp.service_category_id).order("subscription_count DESC").first
      	if @telephone_t_deal.present?
      		@telephone_deal = Deal.where("id = ?", @telephone_t_deal.deal_id).take
      		...
      		@d_call_unlimit = @telephone_deal.domestic_call_unlimited 
      		@i_call_unlimit = @telephone_deal.international_call_unlimited 
      	end
      	...
      	if @cable_t_deal.present?
      		@cable_deal = Deal.where("id = ?", @cable_t_deal.deal_id).take
      	...
      	if @cell_t_deal.present?
      		@cellphone_deal = Deal.where("id = ?", @cell_t_deal.deal_id).take
      		...
      		@d_call_unlimit = @cellphone_deal.domestic_call_unlimited 
      		@i_call_unlimit = @cellphone_deal.international_call_unlimited
      	end
      end  
    end 
