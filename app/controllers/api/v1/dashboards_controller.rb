		...
		if params[:app_user_id].present? && params[:zip_code].present? && params[:category].blank? && params[:sort_by_d_speed].blank? && params[:state].blank?											                	
		  		...
		  		if sp.service_category_id == 1
					...
					@b_deal = @merged_deals.first
		  			...
		  			if sp.cellphone_service_preference.domestic_call_unlimited == true
		  				@equal_deals = Deal.joins(:cellphone_deal_attributes).where("deals.is_active = ? AND deals.service_category_id = ? AND deals.end_date > ? AND cellphone_deal_attributes.domestic_call_minutes='Unlimited' AND deals.price = ?", true, sp.service_category_id, Date.today,@app_user_current_plan).order("price ASC").first
		  				@greater_deals = Deal.joins(:cellphone_deal_attributes).where("deals.is_active = ? AND deals.service_category_id = ? AND deals.end_date > ? AND cellphone_deal_attributes.domestic_call_minutes='Unlimited' AND deals.price > ?", true, sp.service_category_id, Date.today,@app_user_current_plan).order("price ASC").first
		  				@merged_deals = (@equal_deals + @greater_deals).sort_by(&:price)
						@b_deal = @merged_deals.first
		  				if @b_deal.present?
		  					@you_save = '%.2f' % (@app_user_current_plan - @b_deal.price)
		  				...
		  				@equal_deals = Deal.joins(:cellphone_deal_attributes).where("deals.is_active = ? AND deals.service_category_id = ? AND deals.end_date > ? AND cellphone_deal_attributes.domestic_call_minutes = ?", true, sp.service_category_id, Date.today,@app_user_c_minutes).order("price ASC").first
		  				@greater_deals = Deal.joins(:cellphone_deal_attributes).where("deals.is_active = ? AND deals.service_category_id = ? AND deals.end_date > ? AND cellphone_deal_attributes.domestic_call_minutes = ?", true, sp.service_category_id, Date.today,@app_user_c_minutes).order("price ASC").first
		  				@merged_deals = (@equal_deals + @greater_deals).sort_by(&:price)
						@b_deal = @merged_deals.first
		  				if @b_deal.present?
		  					@you_save = '%.2f' % (@app_user_current_plan - @b_deal.price)
			...
			if params[:sorting_flag] == 'download_speed' #For Internet
				...
				if params[:category] == '1'
					...
					@deals = Deal.joins(:cable_deal_attributes).select("deals.*,cable_deal_attributes.free_channels,cable_deal_attributes.premium_channels").where("deals.is_active = ? AND deals.service_category_id = ? AND deals.end_date > ?", true, params[:category], Date.today).order("deals.price ASC")
				elsif params[:category]=='4'
					@deals = Deal.joins(:cellphone_deal_attributes).select("deals.*,cellphone_deal_attributes.domestic_call_minutes,cellphone_deal_attributes.international_call_minutes").where("deals.is_active = ? AND deals.service_category_id = ? AND deals.end_date > ?", true, params[:category], Date.today).order("deals.price ASC")
				elsif params[:category]=='5'
					@deals = Deal.joins(:bundle_deal_attributes).select("deals.*,bundle_deal_attributes.free_channels,bundle_deal_attributes.premium_channels,bundle_deal_attributes.domestic_call_minutes,bundle_deal_attributes.international_call_minutes,bundle_deal_attributes.download as download_speed,bundle_deal_attributes.upload as upload_speed").where("deals.is_active = ? AND deals.service_category_id = ? AND deals.end_date > ?", true, params[:category], Date.today).order("deals.price ASC")
				else
				...
				if params[:category] == '3'
					@deals = Deal.joins(:cable_deal_attributes).select("deals.*,cable_deal_attributes.free_channels,cable_deal_attributes.premium_channels").where("deals.is_active = ? AND deals.service_category_id = ? AND deals.end_date > ?", true, params[:category], Date.today).order("cable_deal_attributes.free_channels DESC")
				elsif params[:category] == '5'
					@deals = Deal.joins(:bundle_deal_attributes).select("deals.*,bundle_deal_attributes.free_channels,bundle_deal_attributes.premium_channels,bundle_deal_attributes.domestic_call_minutes,bundle_deal_attributes.international_call_minutes,bundle_deal_attributes.download as download_speed,bundle_deal_attributes.upload as upload_speed").where("deals.is_active = ? AND deals.service_category_id = ? AND deals.end_date > ?", true, params[:category], Date.today).order("bundle_deal_attributes.free_channels DESC")
				end
				...
				if params[:category] == '2'
					@deals = Deal.joins(:telephone_deal_attributes).select("deals.*,telephone_deal_attributes.domestic_call_minutes,telephone_deal_attributes.international_call_minutes").where("deals.is_active = ? AND deals.service_category_id = ? AND deals.end_date > ?", true, params[:category], Date.today).order("telephone_deal_attributes.domestic_call_minutes DESC")
		  			...
		  			@deals = Deal.joins(:cellphone_deal_attributes).select("deals.*,cellphone_deal_attributes.domestic_call_minutes,cellphone_deal_attributes.international_call_minutes").where("deals.is_active = ? AND deals.service_category_id = ? AND deals.end_date > ?", true, params[:category], Date.today).order("cellphone_deal_attributes.domestic_call_minutes DESC")
				elsif params[:category] == '5'
		  			@deals = Deal.joins(:bundle_deal_attributes).select("deals.*,bundle_deal_attributes.free_channels,bundle_deal_attributes.premium_channels,bundle_deal_attributes.domestic_call_minutes,bundle_deal_attributes.international_call_minutes,bundle_deal_attributes.download as download_speed,bundle_deal_attributes.upload as upload_speed").where("deals.is_active = ? AND deals.service_category_id = ? AND deals.end_date > ?", true, params[:category], Date.today).order("bundle_deal_attributes.domestic_call_minutes DESC")
				end
		  	else
				@deals = Deal.where("is_active = ? AND service_category_id = ? AND end_date > ?", true, params[:category], Date.today).order("price ASC")
			end
		...
		if @app_user.present? && @zip_code.present?
			...
			if params[:service_category_id] == '1'
				...
				if @current_t_plan == true
					...
					@smaller_deals = Deal.joins(:telephone_deal_attributes).select("deals.*,telephone_deal_attributes.domestic_call_minutes,telephone_deal_attributes.international_call_minutes").where("deals.is_active = ? AND deals.service_category_id = ? AND deals.price < ? AND telephone_deal_attributes.domestic_call_minutes < ?", true, params[:service_category_id],@current_plan_price,@user_preference.telephone_service_preference.domestic_call_minutes).order("price ASC").limit(2)
					@greater_deals = Deal.joins(:telephone_deal_attributes).select("deals.*,telephone_deal_attributes.domestic_call_minutes,telephone_deal_attributes.international_call_minutes").where("deals.is_active = ? AND deals.service_category_id = ? AND deals.price > ? AND telephone_deal_attributes.domestic_call_minutes > ?", true, params[:service_category_id],@current_plan_price,@user_preference.telephone_service_preference.domestic_call_minutes).order("price ASC").limit(2)
				end
			elsif params[:service_category_id] == '3'
				@current_plan_price = @user_preference.price
				@current_f_channels = @user_preference.cable_service_preference.free_channels
				@equal_deals = Deal.joins(:cable_deal_attributes).select("deals.*,cable_deal_attributes.free_channels,cable_deal_attributes.premium_channels").where("deals.is_active = ? AND deals.service_category_id = ? AND cable_deal_attributes.free_channels = ?", true, params[:service_category_id],@current_f_channels).order("price ASC")
				@smaller_deals = Deal.joins(:cable_deal_attributes).select("deals.*,cable_deal_attributes.free_channels,cable_deal_attributes.premium_channels").where("deals.is_active = ? AND deals.service_category_id = ? AND cable_deal_attributes.free_channels < ?", true, params[:service_category_id],@current_f_channels).order("price DESC").limit(2)
				@greater_deals = Deal.joins(:cable_deal_attributes).select("deals.*,cable_deal_attributes.free_channels,cable_deal_attributes.premium_channels").where("deals.is_active = ? AND deals.service_category_id = ? AND cable_deal_attributes.free_channels > ?", true, params[:service_category_id],@current_f_channels).order("price ASC").limit(2)
			elsif params[:service_category_id] == '4'
				@current_plan_price = @user_preference.price
				@current_t_plan = @user_preference.cellphone_service_preference.domestic_call_unlimited
				if @current_t_plan == true
					@equal_deals = Deal.joins(:cellphone_deal_attributes).select("deals.*,cellphone_deal_attributes.domestic_call_minutes,cellphone_deal_attributes.international_call_minutes").where("deals.is_active = ? AND deals.service_category_id = ? AND cellphone_deal_attributes.domestic_call_minutes='Unlimited' ", true, params[:service_category_id]).order("price ASC")
					@smaller_deals = Deal.joins(:cellphone_deal_attributes).select("deals.*,cellphone_deal_attributes.domestic_call_minutes,cellphone_deal_attributes.international_call_minutes").where("deals.is_active = ? AND deals.service_category_id = ? AND deals.price < ? AND cellphone_deal_attributes.domestic_call_minutes='Unlimited'", true, params[:service_category_id], @current_plan_price).order("price DESC").limit(2)
					...
					@equal_deals = Deal.joins(:cellphone_deal_attributes).select("deals.*,cellphone_deal_attributes.domestic_call_minutes,cellphone_deal_attributes.international_call_minutes").where("deals.is_active = ? AND deals.service_category_id = ? AND cellphone_deal_attributes.domestic_call_minutes = ?", true, params[:service_category_id]).order("price ASC")
					@smaller_deals = Deal.joins(:cellphone_deal_attributes).select("deals.*,cellphone_deal_attributes.domestic_call_minutes,cellphone_deal_attributes.international_call_minutes").where("deals.is_active = ? AND deals.service_category_id = ? AND deals.price < ? AND cellphone_deal_attributes.domestic_call_minutes < ?", true, params[:service_category_id], @current_plan_price,@current_c_minutes).order("price DESC").limit(2)
					@greater_deals = Deal.joins(:cellphone_deal_attributes).select("deals.*,cellphone_deal_attributes.domestic_call_minutes,cellphone_deal_attributes.international_call_minutes").where("deals.is_active = ? AND deals.service_category_id = ? AND deals.price > ? AND cellphone_deal_attributes.domestic_call_minutes > ?", true, params[:service_category_id], @current_plan_price,@current_c_minutes).order("price ASC").limit(2)
				
				end	
			elsif params[:service_category_id] == '5'
				@app_user_bundle_combo = @user_preference.bundle_service_preference.bundle_combo
				@equal_deals = Deal.joins(:bundle_deal_attributes).select("deals.*,bundle_deal_attributes.free_channels,bundle_deal_attributes.premium_channels,bundle_deal_attributes.domestic_call_minutes,bundle_deal_attributes.international_call_minutes,bundle_deal_attributes.download as download_speed,bundle_deal_attributes.upload as upload_speed").where("deals.is_active = ? AND deals.service_category_id = ? AND bundle_deal_attributes.bundle_combo = ?", true, params[:service_category_id],@app_user_bundle_combo).order("price ASC").limit(5)
			end
