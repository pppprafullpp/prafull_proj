			...
			if @app_user.present? && @zip_code.present?
				@service_preferences = @app_user.service_preferences.order("created_at DESC") 
		  		...
		  		if sp.service_category_id == 1
					...
					if @equal_deals.present? && @greater_deals.present?	&& @smaller_deals.present?	
						@merged_deals = (@equal_deals + @greater_deals + @smaller_deals).sort_by(&:price)
						@b_deal = @merged_deals.first
					elsif @equal_deals.present? && @greater_deals.present?	&& @smaller_deals.blank?	
						@merged_deals = (@equal_deals + @greater_deals).sort_by(&:price)
						@b_deal = @merged_deals.first
					elsif @equal_deals.present? && @smaller_deals.present? && @greater_deals.blank?		
						@merged_deals = (@equal_deals + @smaller_deals).sort_by(&:price)
						@b_deal = @merged_deals.first
					elsif @smaller_deals.present? && @greater_deals.present? && @equal_deals.blank? 	
						@merged_deals = (@smaller_deals + @greater_deals).sort_by(&:price)
						@b_deal = @merged_deals.first
					elsif @equal_deals.present? && @greater_deals.blank? && @smaller_deals.blank?
					...
					elsif @greater_deals.present? && @equal_deals.blank? && @smaller_deals.blank?	
					...
					elsif @smaller_deals.present? && @equal_deals.blank? && @greater_deals.blank?	
						@merged_deals = @smaller_deals
						begin
							@b_deal = @merged_deals.first
						rescue
							@b_deal = @merged_deals
					...
					if @equal_deals.present? && @greater_deals.present?	&& @smaller_deals.present?	
		  			...
		  			if sp.cellphone_service_preference.domestic_call_unlimited == true
		  				@equal_deals = Deal.joins(:cellphone_deal_attributes).where("deals.is_active = ? AND deals.service_category_id = ? AND cellphone_deal_attributes.domestic_call_minutes='Unlimited' AND deals.price = ?", true, sp.service_category_id,@app_user_current_plan).order("price ASC").first
		...
		if @app_user.present? && @zip_code.present?
			...
			if params[:service_category_id] == '1'
				...
				if @current_t_plan == true
					...
					@greater_deals = Deal.joins(:cellphone_deal_attributes).select("deals.*,cellphone_deal_attributes.domestic_call_minutes,cellphone_deal_attributes.international_call_minutes,cellphone_deal_attributes.domestic_text,cellphone_deal_attributes.international_text,cellphone_deal_attributes.data_plan,cellphone_deal_attributes.data_speed,cellphone_deal_attributes.equipment,cellphone_deal_attributes.installation,cellphone_deal_attributes.activation").where("deals.is_active = ? AND deals.service_category_id = ? AND cellphone_deal_attributes.domestic_call_minutes > ?", true, params[:service_category_id],@current_c_minutes).order("price ASC").limit(2)
					@smaller_deals = Deal.joins(:cellphone_deal_attributes).select("deals.*,cellphone_deal_attributes.domestic_call_minutes,cellphone_deal_attributes.international_call_minutes,cellphone_deal_attributes.domestic_text,cellphone_deal_attributes.international_text,cellphone_deal_attributes.data_plan,cellphone_deal_attributes.data_speed,cellphone_deal_attributes.equipment,cellphone_deal_attributes.installation,cellphone_deal_attributes.activation").where("deals.is_active = ? AND deals.service_category_id = ? AND cellphone_deal_attributes.domestic_call_minutes < ?", true, params[:service_category_id],@current_c_minutes).order("price DESC").limit(2)
				end	
