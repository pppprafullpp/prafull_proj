	...
	def index
		...
		if params[:app_user_id].present?
		  ...
		  @servicelist = @service_preferences.map do |sp|
		  	...
		  	#	byebug
		  	#	@p_deal = Deal.where("is_active = ?", true).where("service_category_id = ?", sp.service_category.id).where("service_provider_id = ?", sp.service_provider.id).order("price ASC").first
		  		#{ :service_preference_id => sp.id, :contract_fee => sp.contract_fee, :service_category_name => sp.service_category.name, :deal => @p_deal.as_json(:except => [:created_at, :updated_at, :price, :image], :methods => [:deal_image_url, :deal_price]) }
				#elsif sp.service_provider.is_preferred == false
					#byebug
				#	@b_deal = Deal.where("is_active = ?", true).where("service_category_id = ?", sp.service_category.id).where("service_provider_id = ?", sp.service_provider.id).order("price ASC").first
					#{ :service_preference_id => sp.id, :contract_fee => sp.contract_fee, :service_category_name => sp.service_category.name, :deal => @b_deal }
				#end 

				{ :service_preference_id => sp.id, :contract_fee => sp.contract_fee, :service_category_name => sp.service_category.name } 
		  end	
		  #json = @servicelist.to_json
		  #*************** Working ***********************
