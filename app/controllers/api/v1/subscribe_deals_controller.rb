  ...
  def subscription_info
  	if params[:app_user_id].present? && params[:deal_id].present?
  		...
  		if @subscribe_deal.present? 
  			if @subscribe_deal.active_status == false
  			...
  			elsif @subscribe_deal.active_status == true
  				render :status => 200,
             		 :json => {
                        		:success => true
                        	}	
      		...
      		if @subscribe_deal.save
        		#send_notification
        		format.html { redirect_to subscribe_deals_path, :notice => 'Successfully created' }
        		format.xml  { render :xml => @subscribe_deal, :status => :created, :subscribe_deal => @subscribe_deal }
      		else
        		#raise @deal.errors.inspect
        		format.html { render :action => "new" }
        		format.json  { render :json => @subscribe_deal.errors, :status => :unprocessable_entity }
      		end
    		end
  		end	
  	else
