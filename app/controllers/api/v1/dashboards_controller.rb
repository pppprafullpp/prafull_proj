				...
				@service_preferences = @app_user.service_preferences.order("created_at DESC") 
		  		...
		  		if sp.service_category_id == 1
					...
					if @equal_deals.present? && @greater_deals.present?	
						...
						@merged_deals=Deal.joins(:internet_deal_attributes).where("deals.is_active = ? AND deals.service_category_id = ?", true, sp.service_category_id).order("price ASC")
					end
					begin
		  			...
		  			if sp.telephone_service_preference.domestic_call_unlimited == true
						...
						if @equal_deals.present? && @greater_deals.present?	
							...
							@merged_deals=Deal.joins(:telephone_deal_attributes).where("deals.is_active = ? AND deals.service_category_id = ?", true, sp.service_category_id).order("price ASC")
						end
						begin
							@b_deal = @merged_deals.first
						rescue
					...
					if @equal_deals.present? && @greater_deals.present?	
						...
						@merged_deals=Deal.joins(:cable_deal_attributes).where("deals.is_active = ? AND deals.service_category_id = ?", true, sp.service_category_id).order("price ASC")
					end
					begin
						@b_deal = @merged_deals.first
					rescue
		  			...
		  			if sp.cellphone_service_preference.domestic_call_unlimited == true
		  				...
		  				if @equal_deals.present? && @greater_deals.present?
							...
							@merged_deals=Deal.joins(:cellphone_deal_attributes).where("deals.is_active = ? AND deals.service_category_id = ?", true, sp.service_category_id).order("price ASC")
						end
						begin
							@b_deal = @merged_deals.first
						rescue
	...
	def dashboard_deals
		...
		if @app_user.present? && @zip_code.present?
			...
			if params[:service_category_id] == '1'
				...
				@smaller_deals = Deal.joins(:internet_deal_attributes).select("deals.*,internet_deal_attributes.download as download_speed,internet_deal_attributes.upload as upload_speed,internet_deal_attributes.equipment,internet_deal_attributes.installation,internet_deal_attributes.activation").where("deals.is_active = ? AND deals.service_category_id = ? AND internet_deal_attributes.download < ?", true, params[:service_category_id],@user_preference.internet_service_preference.download_speed).order("price DESC")
